...
const envSchema = z.object({
  ...
  ALLOW_DEMO_AUTH_IN_PRODUCTION: z.enum(['true', 'false']).default('false'),
})
...
export function readConfig(overrides: NodeJS.ProcessEnv = process.env) {
  ...
  if (env.NODE_ENV === 'production' && env.ALLOW_DEMO_AUTH_IN_PRODUCTION !== 'true') {
    throw new Error('Demo authentication is disabled in production. Configure real auth or explicitly set ALLOW_DEMO_AUTH_IN_PRODUCTION=true.')
  }
  return {
